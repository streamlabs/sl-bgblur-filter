...
void* BgBlur::obs_create(obs_data_t* settings, obs_source_t* source)
{
	...
	OnnxInstance::instance().init(modelFilepath.wstring());
...
void BgBlur::obs_video_render(void* data, gs_effect_t* _effect)
{
	...
	*/
	...
	if (!OnnxInstance::instance().update(blurData->source, OnnxModel::CATEGORY_BACKGROUND_INVERSE))
	{
	...
	*/
	...
	gs_texture_t *blurredTexture = BgBlurGraphics::blurBackground(OnnxInstance::instance().m_texrender, OnnxInstance::instance().m_kawaseBlurEffect, blurData->blurBackground,
		OnnxInstance::instance().m_maskWidth, OnnxInstance::instance().m_maskHeight, alphaTexture);
	...
	obs_source_process_filter_tech_end(blurData->source, OnnxInstance::instance().m_maskEffect, 0, 0, blurData->blurBackground > 0 ? "DrawWithBlur" : "DrawWithoutBlur");
...
{
	BgBlurData *blurData = (BgBlurData *)data;
	blurData->blurBackground = obs_data_get_int(settings, "blur_background");
